...
En esta sección se mencionan algunos de los diversos tipos de ataques informáticos que existen. Los ataques a sistemas informáticos se pueden clasificar de distintas maneras según su naturaleza. A continuación, se detallan los más comunes y peligrosos:

\paragraph{Ataques de Denegación de Servicio (DoS)}
Un ataque de Denegación de Servicio tiene como objetivo hacer que un sistema o red no esté disponible para sus usuarios legítimos, interrumpiendo su funcionamiento normal. Estos ataques se llevan a cabo sobrecargando los recursos de un servidor o red con una cantidad excesiva de solicitudes. Un ejemplo de este tipo de ataque es el \textit{Distributed Denial of Service} (DDoS), en el cual múltiples dispositivos comprometidos atacan al mismo tiempo un servidor, sobrecargándolo hasta que se vuelve inoperativo \cite{tanenbaum2009}.

\paragraph{Ataques de Inyección}
...
\paragraph{Ataques de \textit{Phishing}}
...
\paragraph{Ataques de \textit{Malware}}
...
Los \textit{firewalls} de próxima generación (NGFW) como Palo Alto Networks, Check Point o Cisco Firepower, inspeccionan el tráfico de red a un nivel profundo (Deep Packet Inspection - DPI), analizando el contenido de los paquetes más allá de los puertos y protocolos tradicionales. Esto permite identificar y bloquear amenazas sofisticadas, \textit{malware}, y tráfico de aplicaciones maliciosas, además de ofrecer funcionalidades como prevención de intrusiones (IPS) y control de aplicaciones \cite{cosmikal_firewall}.
...
\section{Objetivos del proyecto ¿HACE FALTA TENIENDOLO EN LA INTRODUCCIÓN?}
\section{¿DEBERÍA AÑADIR ALGÚNA SECCIÓN NUEVA?}